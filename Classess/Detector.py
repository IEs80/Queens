...
class Detector:
    ...
    def __init__():
        #initialize grid with only one component
        grid = np.zeros(1)
        #initialize an empty image
        img = np.zeros(1,dtype="uint8")
        #initialize an empty board image
        board = np.zeros(1,dtype="uint8")
        #initialize cols_rows
        cols_rows = 0
    ...
    def open_board(self,dir):
        ...
        #now we use the mask to cropp the image to the size of the white part of the mask (showed better results than cropping the image as is)
        ...
        self.img = self.img[y:y+h, x:x+w] # Croppen image
        cv.imshow("Queens Cropped",self.img)

        #We have to find how many squares we have on the board, to create de grid for solving the problem
        #The gray image can also be used to create the groups (in a simpler way, without actually using color)
        ...
        #2. Threshold to get an image with 0-255 values: all that was black, stays black (0); otherwiese, becomes 255
        ret, thresh = cv.threshold(img_gray,125,255,cv.THRESH_BINARY)
        cv.imshow('Queens thresh', thresh)
        #cv.waitKey(0)

        #now we must determine how many squares we have on the image
        # Contour detection
        countour_t, hierarchies_t = cv.findContours(thresh,cv.RETR_LIST,cv.CHAIN_APPROX_NONE)


        #we will also apply Canny to get Contours, because depending on the image it gets better results
        canny = cv.Canny(self.img,125,175)
        ...
        aux = np.zeros(self.img.shape[:2],dtype="uint8")
        cv.drawContours(aux,countour_c,-1,(255,255,255),1)
        cv.imshow('Canny countours', aux)

        filled_countours_c = np.zeros(self.img.shape[:2],dtype='uint8')
        ...
        #additonal cotours detected in each method
        plus_tresh = 2
        plus_canny = 1
        plus_ct = 3

        #The contour detector picks up two additional contours: the entire image and the large square. => I subtract two from the total to get the actual number of squares.
        ...
        self.col_rows = int(np.sqrt(sqr_cant))
        sqr_cant = self.col_rows*self.col_rows
        print(f'{sqr_cant} squares present in the map')
        #defino un np.array de cols_rows x cols_rows
        grid = np.zeros(shape=[2,self.col_rows,self.col_rows]) #dos dimensiones: una para utilizar como guía de espacio "libre-ocupado" y otra para llevar los colores
        print("\n\tTha map: \n\n")
        print(grid)

        x = range(self.col_rows,0)
        print(x)

        #print(countour_t)
        print(self.img.shape[:2])
        #cantidad columnas
        row_nbr = self.img.shape[0]
        col_nbr = self.img.shape[1]

        #resize the empty board to the size of the loaded image
        self.board = np.resize(self.board,self.img.shape[:2])
        self.board = cv.cvtColor(self.board,cv.COLOR_GRAY2BGR)
    ...
    def get_colours(self,sqr_cant,contour,plus):
        ...
        for cnts in range(sqr_cant+plus):
            ...
            col_index = int(((x_center*self.col_rows)/col_nbr))
            row_index = int(((y_center*self.col_rows)/row_nbr))
